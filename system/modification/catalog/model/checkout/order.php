			...
			foreach ($data['products'] as $product) {
                ...
                if (!empty($product['quantity'])) {
                    if (in_array($data['order_status_id'], $this->config->get('config_complete_status'))) {
                        $this->db->query("UPDATE " . DB_PREFIX . "isearch_cache SET sales_amount = (sales_amount + " . (int)$product['quantity'] . ") WHERE product_id = '" . (int)$product['product_id'] . "'");
                    }
                    
                    $this->db->query("UPDATE " . DB_PREFIX . "isearch_cache SET orders_amount = (orders_amount + " . (int)$order_product['quantity'] . ") WHERE product_id = '" . (int)$order_product['product_id'] . "'");
                }
		...
		if ($order_info) {
			...
			$this->db->query("UPDATE `" . DB_PREFIX . "order` SET order_status_id = '" . (int)$order_status_id . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");
                ...
                if (!empty($order_status_id)) {
                    if (in_array($order_status_id, $this->config->get('config_complete_status'))) {
                        $order_product_results = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product WHERE order_id='" . (int)$order_id . "'");
                        foreach ($order_product_results->rows as $order_product) {
                            $this->db->query("UPDATE " . DB_PREFIX . "isearch_cache SET sales_amount = (sales_amount + " . (int)$order_product['quantity'] . ") WHERE product_id = '" . (int)$order_product['product_id'] . "'");
                        }
                    }
                }
			...
			if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
                ...
                if (!empty($order_product['product_id'])) {
                    $new_product_quantity = $this->db->query("SELECT quantity FROM " . DB_PREFIX . "product WHERE product_id='" . (int)$order_product['product_id'] . "'");
                    if ($new_product_quantity->num_rows) {
                        $this->db->query("UPDATE " . DB_PREFIX . "isearch_cache SET in_stock = '" . (bool)$new_product_quantity->row['quantity'] . "', quantity = '" . (int)$new_product_quantity->row['quantity'] . "' WHERE product_id = '" . (int)$order_product['product_id'] . "'");
                    }
                }
			...
			if (in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && !in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
                ...
                if (!empty($product['product_id'])) {
                    $new_product_quantity = $this->db->query("SELECT quantity FROM " . DB_PREFIX . "product WHERE product_id='" . (int)$product['product_id'] . "'");
                    if ($new_product_quantity->num_rows) {
                        $this->db->query("UPDATE " . DB_PREFIX . "isearch_cache SET in_stock = '" . (bool)$new_product_quantity->row['quantity'] . "', quantity = '" . (int)$new_product_quantity->row['quantity'] . "' WHERE product_id = '" . (int)$product['product_id'] . "'");
                    }
                }
