...
class ControllerModuleIsearch extends Controller {
    ...
    $data['heading_title'] = $this->language->get('heading_title');
		...
		if($data['data']['iSearch']['Enabled'] != 'no') {
			$this->document->addScript('catalog/view/javascript/isearch_corporate.js');
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/isearch.tpl')) {
				$template = $this->config->get('config_template') . '/template/module/isearch.tpl';
			} else {
				$template = 'default/template/module/isearch.tpl';
			}
	...
	public function ajaxget() {
		...
		//{HOOK_SEARCH_IN_CACHE_SETTING}
                ...
                $searchInCache = ($set['EnableCaching']=='Yes') ? true : false;
                $showCategories = !empty($set['EnableCategoriesInstant']) && $set['EnableCategoriesInstant'] != 'No';
                $categoriesLimit = !empty($set['CategoryCountInstant']) ? (int)$set['CategoryCountInstant'] : 5;
		...
		$resultsByNameAndModel = $this->model_catalog_isearch->iSearch($keywords,$searchIn,$useImages,$strictSearch,$singularize,$checkRules,'','ASC',0,false,$excludeTerms,$excludeRules,$searchInCache, false, $showCategories, $categoriesLimit);
