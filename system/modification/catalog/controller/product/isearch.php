    ...
    $data['button_search'] = $this->language->get('button_search');
		...
		if (isset($this->request->get['search'])) {
			...
			//{HOOK_SEARCH_IN_CACHE_SETTING}
                ...
                $searchInCache = ($isearch_settings['EnableCaching'] == 'Yes') ? true : false;
                $showCategories = !empty($isearch_settings['EnableCategoriesInstant']) && $isearch_settings['EnableCategoriesInstant'] != 'No';
                $categoriesLimit = 5000; //!empty($isearch_settings['CategoryCountStandard']) ? (int)$isearch_settings['CategoryCountStandard'] : 5;
            
			$start = ($page - 1) * $limit;
			$results = $this->model_catalog_isearch->iSearchStandard($search,$searchIn,$strictSearch,$singularize,$checkRules,$sort,$order,$category_id,$sub_category,$excludeTerms,$excludeRules,$searchInCache,!empty($isearch_settings['SearchIn']['Description']),$showCategories,$categoriesLimit);
		
			$product_total = count($results);

                $product_total = $results['total_products'];
                $data['search_categories'] = $results['categories'];
                $data['total_search_categories'] = $results['total_categories'];
                $results = $results['products'];

                $data['isearch_categories_show'] = !empty($isearch_settings['EnableCategoriesStandard']) && $isearch_settings['EnableCategoriesStandard'] == 'Yes';
                $data['isearch_categories_product_count'] = !empty($isearch_settings['ShowProductCountStandard']) && $isearch_settings['ShowProductCountStandard'] == 'Yes';
                $data['isearch_categories_heading'] = !empty($isearch_settings[$this->config->get('config_language_id')]['CategoryHeadingStandard']) ? $isearch_settings[$this->config->get('config_language_id')]['CategoryHeadingStandard'] : '';
                $data['isearch_categories_show_more'] = !empty($isearch_settings[$this->config->get('config_language_id')]['ShowMoreTextStandard']) ? $isearch_settings[$this->config->get('config_language_id')]['ShowMoreTextStandard'] : '';
                $data['isearch_categories_show_less'] = !empty($isearch_settings[$this->config->get('config_language_id')]['ShowLessTextStandard']) ? $isearch_settings[$this->config->get('config_language_id')]['ShowLessTextStandard'] : '';
                $data['isearch_categories_limit'] = !empty($isearch_settings['CategoryCountStandard']) ? (int)$isearch_settings['CategoryCountStandard'] : 5;
			...
			foreach ($results as $result) {
				...
				$data['products'][] = array(
					...
					'thumb'       => $image,
					'name'        => $result['name'],
					'description' => $this->model_catalog_isearch->isearch_substr(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8')), 0, $this->config->get('config_product_description_length')) . '...',
					'price'       => $price,
					'model'				=> $result['model'],
					...
					'href'        => $this->url->link('product/product', $url . '&product_id=' . $result['product_id'], 'SSL')
				);
			}
			...
			$data['limits'][] = array(
				...
				'href'  => $this->url->link('product/isearch', $url . '&limit=' . $this->config->get('config_product_limit'))
			);
